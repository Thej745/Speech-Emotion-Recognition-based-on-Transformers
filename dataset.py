...
class SpeechDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        #return input, label
        offset = np.random.uniform(0, max(0, librosa.get_duration(path=path) - self.segment_length)) 
        speech = librosa.load(path, sr=self.tsr, offset=offset, duration=self.segment_length)[0]
        input_features = self.feature_extractor(speech, sampling_rate=self.tsr)
        input_values = input_features.input_values[0]  
...
def get_data_loaders(train_data, val_data , train_bs  , fe):
    ...
    val_dl = DataLoader(SpeechDataset(val_data , fe ) , batch_size = train_bs, collate_fn = collate_fn, shuffle=True)
